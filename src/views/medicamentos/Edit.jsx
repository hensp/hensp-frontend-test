...
const Edit = () => {
  ...
  const updateMedicamento= async(e) =>{
    e.preventDefault();
    const form = {
      nombre:nombre,
      proveedor:proveedor,
    ...
    await axios.put("/medicamentos/"+params.id,form).then(
      response =>{
          res = response.data,
          (response.status==200)?(Swal.fire({title:"Registro actualizado con exito! ",icon:'success',buttonsStyling:true})):""
      ...
      <div className="col-md-6">
          <div aria-label="breadcrumb">
            ...
            <Link to={"/"} className="breadcrumb-item" ><i className="fa-solid fa-clipboard-list"></i> Medicamentos</Link>
              <li className="breadcrumb-item active" aria-current="page"><i className="fa-solid fa-prescription-bottle-medical"></i> Actualizar Medicamento</li>
      ...
      <div className="col-md-4 col-lg-4 col-sm-6 mt-4">
        <div className="card border border-warning">
          <div className="card-header">
            <h5 className="card-title">Actualizar Medicamento</h5>
          </div>
          <div className="card-body">
          
            <form className="mt-4" onSubmit={updateMedicamento}>
              ...
              <button type="submit" className="btn btn-warning mt-4">Actualizar</button>
            </form>
          </div>
        </div>
      </div>
